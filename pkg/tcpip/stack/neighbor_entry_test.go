...
func TestEntryFailedToFailed(t *testing.T) {
	...
	if got := e.nic.stats.Neighbor.FailedEntryLookups.Value(); got != 0 {
		t.Errorf("got Neighbor.QueuedInFailed = %d, want = 0", got)
	}
	...
	if got := e.nic.stats.Neighbor.FailedEntryLookups.Value(); got != 1 {
		t.Errorf("got Neighbor.QueuedInFailed = %d, want = 1", got)
	}
}
