...
type igmpState struct {
	...
	mu struct {
		...
		// RFC 2236 Section 4 Page 6: "The IGMPv1 router expects Version 1 Membership
		// Reports in response to its Queries, and will not pay attention to Version
		// 2 Membership Reports.  Therefore, a state variable MUST be kept for each
		// interface, describing whether the multicast Querier on that interface is
		// running IGMPv1 or IGMPv2.  This variable MUST be based upon whether or not
		// an IGMPv1 query was heard in the last [Version 1 Router Present Timeout]
		// seconds"
		igmpV1Present bool
...
func (igmp *igmpState) handleMembershipQuery(groupAddress tcpip.Address, maxRespTime byte) {
	...
	if groupAddress == header.IPv4Any {
		for membershipAddress, info := range igmp.mu.memberships {
			igmp.setDelayTimerForAddress(membershipAddress, &info, maxRespTime)
			igmp.mu.memberships[membershipAddress] = info
		}
	} else if info, ok := igmp.mu.memberships[groupAddress]; ok {
		igmp.setDelayTimerForAddress(groupAddress, &info, maxRespTime)
		igmp.mu.memberships[groupAddress] = info
	}
}

// setDelayTimerForAddress modifies the passed info only and does not modify
// IGMP state directly, so it does not need to acquire the lock itself.
//
// The IGMP mutex MUST be locked.
func (igmp *igmpState) setDelayTimerForAddress(groupAddress tcpip.Address, info *membershipInfo, maxRespTime byte) {
...
// writeIGMPPacket assembles and sends an IGMP packet with the provided fields,
// incrementing the provided stat counter on success.
func (igmp *igmpState) writeIGMPPacket(destAddress tcpip.Address, groupAddress tcpip.Address, igmpType header.IGMPType, statCounter func(tcpip.IGMPSentPacketStats) *tcpip.StatCounter) {
...
//
// The IGMP mutex MUST be locked.
func (igmp *igmpState) sendReport(groupAddress tcpip.Address) {
	var igmpType header.IGMPType
	var statCounter func(tcpip.IGMPSentPacketStats) *tcpip.StatCounter
	if igmp.mu.igmpV1Present {
		igmpType = header.IGMPv1MembershipReport
		statCounter = func(stats tcpip.IGMPSentPacketStats) *tcpip.StatCounter {
	...
	igmp.writeIGMPPacket(groupAddress, groupAddress, igmpType, statCounter)
...
//
// The IGMP mutex MUST be locked.
func (igmp *igmpState) sendLeave(groupAddress tcpip.Address) {
	...
	igmp.writeIGMPPacket(header.IPv4AllRoutersGroup, groupAddress, header.IGMPLeaveGroup, func(stats tcpip.IGMPSentPacketStats) *tcpip.StatCounter {
...
func (igmp *igmpState) joinGroup(groupAddress tcpip.Address) *tcpip.Error {
	...
	igmp.sendReport(groupAddress)
	igmp.setDelayTimerForAddress(groupAddress, &info, UnsolicitedReportIntervalMaxTenthSec)
	igmp.mu.memberships[groupAddress] = info
