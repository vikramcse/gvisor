...
func getD(VD vfs.VirtualDentry) *dentry {
	return VD.Dentry().Impl().(*dentry)
}
...
func getDentry(fd *vfs.FileDescription) *dentry {
	return fd.Impl().(*fileDescription).d
}
...
func newFileFD(ctx context.Context, vfsObj *vfs.VirtualFilesystem, root vfs.VirtualDentry, filePath string, mode linux.FileMode) (*vfs.FileDescription, int, error) {
	...
	lowerFD, err := getD(root).openLowerAt(ctx, vfsObj, filePath, linux.O_RDWR|linux.O_CREAT|linux.O_EXCL, linux.ModeRegular|mode)
	if err != nil {
		return nil, 0, err
	}
...
func TestOpen(t *testing.T) {
	for _, alg := range hashAlgs {
		...
		if _, err = getDentry(fd).openLowerMerkleAt(ctx, vfsObj, linux.O_RDONLY, linux.ModeRegular); err != nil {
			t.Errorf("OpenAt Merkle tree file %s: %v", merklePrefix+filename, err)
		}

		// Ensure the root merkle tree file is created.
		if _, err = getD(root).openLowerMerkleAt(ctx, vfsObj, linux.O_RDONLY, linux.ModeRegular); err != nil {
...
func TestPReadModifiedFileFails(t *testing.T) {
	for _, alg := range hashAlgs {
		...
		lowerFD, err := getDentry(fd).openLowerAt(ctx, vfsObj, "", linux.O_RDWR, linux.ModeRegular)
		if err != nil {
			t.Fatalf("OpenAt: %v", err)
		}
...
func TestModifiedMerkleFails(t *testing.T) {
	for _, alg := range hashAlgs {
		...
		lowerMerkleFD, err := getDentry(fd).openLowerMerkleAt(ctx, vfsObj, linux.O_RDWR, linux.ModeRegular)
		if err != nil {
			t.Fatalf("OpenAt: %v", err)
		}
...
func TestModifiedParentMerkleFails(t *testing.T) {
	for _, alg := range hashAlgs {
		...
		// Open a new lowerMerkleFD that's read/writable.
		parentLowerMerkleFD, err := getDentry(fd).parent.openLowerMerkleAt(ctx, vfsObj, linux.O_RDWR, linux.ModeRegular)
		if err != nil {
			t.Fatalf("OpenAt: %v", err)
		}
...
func TestOpenDeletedFileFails(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.changeFile {
				if err := getD(root).unlinkLowerAt(ctx, vfsObj, filename); err != nil {
					t.Fatalf("UnlinkAt: %v", err)
				}
			}
			if tc.changeMerkleFile {
				if err := getD(root).unlinkLowerMerkleAt(ctx, vfsObj, filename); err != nil {
					t.Fatalf("UnlinkAt: %v", err)
				}
			}
...
func TestOpenRenamedFileFails(t *testing.T) {
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			if tc.changeFile {
				if err := getD(root).renameLowerAt(ctx, vfsObj, filename, newFilename); err != nil {
					t.Fatalf("RenameAt: %v", err)
				}
			}
			if tc.changeMerkleFile {
				if err := getD(root).renameLowerMerkleAt(ctx, vfsObj, filename, newFilename); err != nil {
					t.Fatalf("UnlinkAt: %v", err)
				}
			}
